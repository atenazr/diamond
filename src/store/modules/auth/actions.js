...
export default {
  ...
  async sendMessage({commit,getters},payload){
    const mesg = payload.message;
    const eml = payload.email;
    const userId = getters['userId'];
    console.log('message',userId);

    const response = await fetch(`https://golden-price-57b97-default-rtdb.firebaseio.com/${userId}.json`,{
      method:'PUT',
      body:JSON.stringify({
        message:mesg,
        email:eml
    ...
    if(!(response).ok){
      const error = response.error || 'sending message is failure !'
      throw error
    }
